const Minifrdg = (rootSelector) => {
  ...
  const fill = (template, ctrl) => {return template.replace(/\{\{(.+?)\}\}/g, (all, str) => {const r = (new Function("try{with(this) {return " + (/&.+?;/.test(str) && (text.innerHTML = str) && text.innerText || str) + "}}catch(e){return ''}")).call(ctrl);return ['undefined','null'].includes(typeof(r))?'':r})};
