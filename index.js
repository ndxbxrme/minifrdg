const Minifrdg = (rootSelector) => {
  ...
  const fill = (template, ctrl) => {return template.replace(/\{\{(.+?)\}\}/g, (all, str) => (new Function("try{with(this) {return " + (/&.+?;/.test(str) && (text.innerHTML = str) && text.innerText || str) + "}}catch(e){return ''}")).call(ctrl))};
