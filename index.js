const Minifrdg = (rootSelector) => {
  ...
  const fill = (template, ctrl) => {const result = template.replace(/\n/g,'/*NL*/').replace(/\{\{(.+?)\}\}/gm, (all, str) => {const r = (new Function("try{with(this) {return " + (/&.+?;/.test(str) && (text.innerHTML = str) && text.innerText || str).replace(/^#/,'') + "}}catch(e){return ''}")).call(typeof(ctrl)==='object' && Object.assign(ctrl,{app:app})||ctrl); return ['undefined','null'].includes(typeof(r))?'':(/app\.fill|^#/.test(str)?r:app.safe(r))}); /<[^>]*\s(href=|on)[^>]+>/.test(result) && hookActions(result); return result.replace(/\/\*NL\*\//g,'\n')};
